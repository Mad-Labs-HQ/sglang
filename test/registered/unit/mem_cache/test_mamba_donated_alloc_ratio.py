...
class TestChunkedStashOnExhaustedPool(unittest.TestCase):
    ...
    def _donate(req, new_slot):
        ...
        keep = req.kv.mamba_last_track_idx
        donated = req.kv.mamba_ping_pong_track_buffer[keep].unsqueeze(-1).clone()
        req.kv.mamba_ping_pong_track_buffer[keep] = new_slot[0]
        return donated
    ...
    def test_extra_buffer_stash_skips_when_pool_exhausted(self):
        ...
        buffer_before = req.kv.mamba_ping_pong_track_buffer.clone()
    ...
    def test_extra_buffer_stash_donates_with_headroom(self):
        ...
        tracked = req.kv.mamba_ping_pong_track_buffer[0].item()
        insert_params = SimpleNamespace(mamba_value=None)
        ...
        self.assertNotEqual(req.kv.mamba_ping_pong_track_buffer[0].item(), tracked)
    ...
    def test_no_buffer_stash_skips_when_pool_exhausted(self):
        ...
        req = SimpleNamespace(
            rid="chunked", kv=SimpleNamespace(mamba_pool_idx=owned[0][0])
        )
        insert_params = SimpleNamespace(mamba_value=None)
