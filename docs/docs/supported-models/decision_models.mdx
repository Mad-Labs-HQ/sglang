...
The probabilities of a chat model's answer labels are not calibrated: an answer given 0.9 is not right 90% of the time. Models trained after instruction tuning tend to be overconfident, their label choices shift with the order options are shown in and with the letters they carry, and many put most of their probability on a spelling of the label the prompt did not ask for, such as `Yes` for `yes`. `/v1/systemone` can correct for this in three modes, chosen per request with the `x_calibration` extension or by default from the server's `--decision-calibration-config`:

| Mode | Needs | Answers |
| --- | --- | --- |
| `raw` | nothing, the default without a config | One read per question, exactly as `/v1/decisions` reports it |
| `label_free` | a config | Several reads per question, combined without any labeled data |
| `fitted` | a config with fitted profiles | The reads of its base mode, then a temperature (choice and score) or Platt scaling (noul) fitted on your labeled rows |

A read is one prompt of a question. `label_free` makes these reads:

- **noul in both orders** (`noul_orders: 2`): once asking for yes or no, once for no or yes, averaged in log space, which cancels a preference for the first answer offered.
- **noul case variants** (`noul_case_variants`): `Yes`, `YES`, `No`, and `NO` count for their answer where each is one distinct token, which recovers the probability a model puts on spellings the prompt did not ask for.
- **choice rotations** (`choice_rotations: k`): the options shown in `k` evenly spaced cyclic orders with the letters kept in place, combined by geometric mean, which averages out a preference for particular letters.
- **choice name variants** (`choice_name_variants`): the first token of an option's name also counts for it, when no other option or label has that token.
- **batch prior** (`batch_prior`): a running mean of each question's answers, divided out at a strength once `min_count` answers are seen, as in Batch Calibration. It is kept per question and per server process, so answers depend on the traffic before them, and it hurts questions whose true answers are skewed.

Score levels are ordinal and always shown in order. The config file lists every setting, and a missing or unknown field is refused at startup:
...
This is `examples/runtime/systemone/label_free.json`, the components that lowered the mean calibration error on the benchmark below without costing any task more than a point of accuracy. To fit profiles, launch with it, write your labeled rows as System One request bodies with a `labels` map (an option name, a level index, or `true` or `false` per question id), and run:

```bash Command
python -m sglang.srt.entrypoints.systemone.fit_calibration --url http://127.0.0.1:30000 \
  --config label_free.json --base label_free --data labeled.jsonl \
  --min-rows 50 --folds 5 --out fitted.json
```

The tool fits one profile per question with at least `--min-rows` rows, one per question type and option count, and one per type, and keeps a profile only when it lowers the out-of-fold NLL. A question uses its most specific profile, and answers with no profile report the base mode in `x_calibration`. Relaunch with `--decision-calibration-config fitted.json`. The file records the model, revision, prompt format version, and reads it was fitted on, and a server with any of them different refuses it at startup.

Each answer reports the mode applied in `x_calibration`. With `x_return_reads: true`, `x_reads` lists every read with its option order, labels, and the full-vocabulary logprobs of the tokens counted for each option, from which the answer can be recomputed with `sglang.srt.entrypoints.systemone.calibration`.

On `IFM/K2-Horizon-3.7B`, over eight public tasks with 300 test rows each, mean accuracy and expected calibration error (ECE) per question type as served. `raw` comes from the collection run and the other modes from their own runs, which differ from it only by the drift described below (see `benchmark/systemone/README.md` for every number):

| Task type | `raw` accuracy / ECE | `label_free` | `fitted` on 300 rows per task |
| --- | --- | --- | --- |
| noul (BoolQ, SST-2, AG News sports) | 0.703 / 0.231 | 0.861 / 0.148 | 0.874 / 0.068 |
| choice (AG News, emotion, Banking77) | 0.650 / 0.114 | 0.649 / 0.116 | 0.649 / 0.075 |
| score (SST-5, Yelp) | 0.275 / 0.220 | 0.268 / 0.223 | 0.264 / 0.040 |

What to keep in mind:

- Only fitted profiles make probabilities trustworthy for thresholds, and only for the kind of question they were fitted on: fitted temperatures differ between tasks, and one fitted on one task can raise the error on another.
- Choice rotations raised K2-Horizon's accuracy by up to 12 points and lowered its NLL and Brier score on every choice task, but raised the calibration error of the emotion task, so the example config leaves them off. Turn them on when accuracy matters more than calibration without labels, or fit a profile on top of them.
- Content-free priors (asking each question about `N/A`) and the batch prior were measured and not kept: both lowered accuracy on skewed tasks.
...
- Answers move slightly with batch composition and prefix caching, as described in [Reproducibility](#reproducibility). On K2-Horizon-3.7B, two runs of the same label-free reads differed by 0.001 to 0.013 in mean probability and 0.02 to 0.08 at the 99th percentile, and the top answer changed on 0.7% to 2.7% of noul and choice questions and on 7% of the score questions, whose level probabilities are flat.
