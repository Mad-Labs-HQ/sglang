...
- `usage.input_tokens` counts the prompt of every read, so the state is counted once per read of every question, and `usage.output_tokens` is 0.
...
- Reads multiply the prompts per question (two for noul, `k` for choice).
