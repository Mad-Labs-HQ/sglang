...
- `choice_rotations: k` shows the options in `k` evenly spaced cyclic orders with the letters kept in place, which averages out a preference for particular letters.
- `choice_name_variants` counts the first token of an option's name for it, when no other option or label has that token.

Score levels are ordinal and always read once, in order. The reads of a question are combined by geometric mean. A request without `x_read_setup`, such as one from a System One SDK, gets the server's default reads, set with `--decision-reads-config`; without that flag every question gets the single read of `/v1/decisions`. The config lists every field and caps the rotations a request may ask for:
...
This is `examples/runtime/systemone/reads.json`: the reads that lowered the mean calibration error of K2-Horizon-3.7B on the benchmark below without costing any task more than a point of accuracy. Without the flag, requests may ask for up to 8 rotations.
...
Every answer carries `x_read_probabilities`, its option probabilities before any calibration, and `x_fingerprint`, which identifies the model, the prompt wording, and the parts of the read setup that apply to the question. A calibration may send the fingerprint of the answers it was fitted on as `fitted_on`; the server then refuses it once the model or those reads change, instead of calibrating with it. Rotations change the fingerprint of choice questions only, and yes or no orders and case variants that of noul questions only.
...
- Choice rotations raised K2-Horizon's accuracy by up to 12 points and lowered its NLL and Brier score on every choice task, but raised the calibration error of the emotion task without a calibration, so `reads.json` leaves them off by default. Ask for them per request, and fit calibrations with the same reads.
...
- Reads multiply the prompts per question (two for noul, `k` for choice), but they share the state, which is prefilled once per request.
