...
| **K2-Horizon-3.7B** | `IFM/K2-Horizon-3.7B` | Validated on one RTX 5070 Ti (16 GB) in BF16 with `--attention-backend flashinfer`. The chat template always reasons, so answers are read after an empty reasoning block, see [Thinking models](#thinking-models). See the [model page](/cookbook/autoregressive/IFM/K2-Horizon). |
...
- `chat_template_kwargs` that set the thinking toggle to anything but `false`
- a chat template that always thinks before answering, starts every answer with a reasoning block, or leaves a reasoning block open in the rendered prompt, or a model whose reasoning parser expects answers to start inside a reasoning block when the rendered prompt does not close one, in each case only when the template's own reply without reasoning does not close an empty reasoning block either, as described in [Thinking models](#thinking-models)
...
Qwen3.8-27B and Qwen3.5-35B-A3B think by default. `/v1/decisions` turns thinking off for every question and refuses a request that turns it back on, so the answer is never read inside the reasoning. Chat requests to the same server keep the model's default, and launching with `--reasoning-parser qwen3` affects chat only.

Some chat templates cannot turn thinking off: K2-Horizon's opens a reasoning block (`<ifm|think>`) in every generation prompt. When the rendered prompt would put the answer inside a reasoning block, the server renders the template's own finished reply with empty reasoning and, if that reply continues the generation prompt and closes one empty block followed only by whitespace before the answer, extends the prompt by that text. For K2-Horizon the prompt then ends with `<ifm|think>\n</ifm|think>`, which is how the template itself renders an answer without reasoning. Answers read this way skip the reasoning the model would otherwise do, so expect lower accuracy than chat with reasoning on hard questions.

The reply is rendered with an empty reasoning field, trying `reasoning_content`, `reasoning`, `thinking`, and `think` in that order until one closes a block, so K2-Horizon answers after the `<ifm|think>` pair. A `reasoning_effort` other than `high` in `chat_template_kwargs` or `--default-chat-template-kwargs` opens a different pair in the generation prompt, which that reply does not continue, and the request is refused.
...
Models with full attention only drift too. On K2-Horizon-3.7B in BF16, answers to the same prompts in different batches and cache states moved by 0.001 to 0.013 on average, and near ties changed their top answer, as measured under [Calibration](#calibration). The decision routes prefill a prefix shared by all prompts of a request once before scoring them, so every question of a request reads its state from the cache.
...
- `confidence` follows the formulas TypeSafe publishes for its adapter. It is a statistic of the returned probabilities, which are renormalized over the answer labels and ignore probability outside them. It can read 0 for a score split between distant levels, and thresholds tuned on another provider may not transfer. Answers are not calibrated unless the request sends a calibration, see [Calibration](#calibration).
- Each answer also carries `x_label_mass`, the full-vocabulary probability of the tokens scored for its options in the read that shows them in request order, which `/v1/decisions` returns as `label_mass` for its labels, and the [calibration](#calibration) fields `x_read_probabilities`, `x_calibration`, and `x_fingerprint`. With `x_return_reads`, it also carries `x_reads`. The Python SDK drops fields it does not define, so read them from `result.raw_http_response.json()`.
...
- Unknown top-level fields are ignored. Unknown keys inside a question or inside `noul` criteria are refused, and so are the `/v1/decisions` fields `temperature`, `prompt_format_version`, and `return_prompt_token_ids` with any value other than null. `chat_template_kwargs`, `x_read_setup`, and `x_return_reads` on the request and `x_calibration` on a question are accepted as SGLang extensions.
...
- `usage.input_tokens` counts the prompt of every read, so the state is counted once per read of every question, and `usage.output_tokens` is 0. A prefix shared by all reads of a request is prefilled once when it is at least `SGLANG_DECISION_PREFIX_PRIME_MIN_TOKENS` tokens (256 by default), so the state is computed once however many questions ask about it.
...
The probabilities of a chat model's answer labels are not calibrated: an answer given 0.9 is not right 90% of the time. Models trained after instruction tuning tend to be overconfident, their label choices shift with the order options are shown in and with the letters they carry, and many put most of their probability on a spelling of the label the prompt did not ask for, such as `Yes` for `yes`. `/v1/systemone` corrects for this in two steps, both chosen by the client: how each question is read, and a calibration the client fitted for the question on its own labeled data. The server keeps no calibration state.

### Reads

A read is one prompt of a question. `x_read_setup` on the request chooses them:
...
- `choice_orders` and `choice_max_orders` show a choice question's options in several orders, the request order first, with the letters kept in place, which averages out a preference for particular letters or positions. `"rotations"` are the cyclic shifts of the options; `"williams"` are the rows of a Williams square. With `choice_max_orders: "all"`, or any number at least the option count, a question with n options is read in n orders, and every option appears in every position once. Williams rows also put every ordered pair of options next to each other once (for an even n), so the answer depends least on the order the request lists the options in; with rotations, each option keeps the same neighbors. With a smaller number, the question is read in that many orders, the most discordant subset of the n: each order added is the one whose pairwise option order disagrees most with the orders already chosen.
- `noul_orders: 2` asks each yes or no question twice, once for yes or no and once for no or yes, which cancels a preference for the first answer offered.
- `noul_case_variants` counts `Yes`, `YES`, `No`, and `NO` for their answer where each is one distinct token.
- `choice_name_variants` counts the first token of an option's name for it, when no other option or label has that token.

Score levels are ordinal and always read once, in order. The reads of a question are combined by geometric mean, and they share the state, which is prefilled once per request. A request without `x_read_setup`, such as one from a System One SDK, gets the server's default reads, set with `--decision-reads-config`; without that flag every question gets the single read of `/v1/decisions`. The config lists every field, and `max_choice_orders` caps the orders any choice question may be read in: a question that a request would read in more orders is refused, and the default reads must name a number within the cap.
...
This is `examples/runtime/systemone/reads.json`: by default one order per choice question, and yes or no questions in both orders with every case of the labels, the reads that lowered the mean calibration error of K2-Horizon-3.7B on the benchmark below without costing any task more than a point of accuracy. Without the flag, requests may ask for up to 8 orders.

### Calibrations

Each question can carry an `x_calibration`, applied to the log probabilities of its combined reads:

| `type` | Question types | Fields | Effect |
| --- | --- | --- | --- |
| `temperature` | all | `temperature` | Divides every log probability |
| `platt` | noul | `a`, `b` | P(yes) = sigmoid(a * (log P(yes) - log P(no)) + b) |
| `vector` | choice, score | `scale`, `bias`, one per option | Scales and shifts each option's log probability (diagonal Dirichlet calibration), which also corrects a bias toward particular options |
...
"questions": {
  "urgent": {
    ...
    "x_calibration": {"type": "platt", "a": 3.14, "b": 1.28, "fitted_on": "c1af44e59041d84e"}
  }
}
```

Every answer carries `x_read_probabilities`, its option probabilities before any calibration, and `x_fingerprint`, which identifies the model, the prompt wording, the option count, and the reads the question gets. A calibration may send the fingerprint of the answers it was fitted on as `fitted_on`; the server then refuses it once the model or those reads change, instead of calibrating with it. Choice orders change the fingerprint of choice questions only, and yes or no orders and case variants that of noul questions only. The fingerprint follows the reads, not their spelling: `"all"` and any larger number give the same one.

Fit calibrations from labeled rows written as System One request bodies with a `labels` map (an option name, a level index, or `true` or `false` per question id), where a question id always names the same question:

```bash Command
python -m sglang.srt.entrypoints.systemone.fit_calibration --url http://127.0.0.1:30000 \
  --data labeled.jsonl --read-setup reads.json \
  --min-rows 50 --folds 5 --concurrency 8 --out calibrations.json
```

The tool runs on the client: it asks the server each row with the read setup (the server's default reads when `--read-setup` is absent), fits the candidates for each question id with at least `--min-rows` labeled answers (temperature and Platt scaling for noul, temperature and vector scaling for choice and score), and keeps the one with the lowest out-of-fold NLL, or none when leaving the answers as they are does better. `calibrations.json` maps question ids to calibrations stamped with `fitted_on`. Send each as its question's `x_calibration`, with the same `x_read_setup`.

Each answer reports the calibration applied in `x_calibration` (`none` without one). With `x_return_reads: true`, `x_reads` lists every read with its option order, labels, and the full-vocabulary logprobs of the tokens counted for each option, from which the answer can be recomputed with `sglang.srt.entrypoints.systemone.calibration`.

### Measured

On `IFM/K2-Horizon-3.7B`, over eight public tasks with 300 test rows each, mean accuracy and expected calibration error (ECE) per question type as served. The default reads are those of `reads.json`; calibrations were fitted by the tool on 300 other rows per task. `raw` comes from the collection run and the others from their own runs, which differ from it only by the drift described below (see `benchmark/systemone/README.md` for every number):

| Task type | Single read, accuracy / ECE | Default reads | Default reads, calibrated |
| --- | --- | --- | --- |
| noul (BoolQ, SST-2, AG News sports) | 0.703 / 0.231 | 0.866 / 0.147 | 0.874 / 0.056 |
| choice (AG News, emotion, Banking77) | 0.650 / 0.114 | 0.648 / 0.118 | 0.680 / 0.057 |
| score (SST-5, Yelp) | 0.275 / 0.220 | 0.267 / 0.230 | 0.303 / 0.060 |

What to keep in mind:

- Only fitted calibrations make probabilities trustworthy for thresholds, and only for the question they were fitted on: the fit tool chose Platt scaling, temperature, or vector scaling per question, and its temperatures ran from 0.5, where K2-Horizon was underconfident, to 5.4.
- Vector scaling also raised accuracy where a model prefers some options regardless of the input (emotion 0.46 to 0.52, Banking77 0.60 to 0.63, SST-5 0.27 to 0.35).
- Reading choice questions in several orders raised K2-Horizon's accuracy by up to 12 points and lowered its NLL and Brier score on every choice task, but raised the calibration error of the emotion task without a calibration, so `reads.json` reads one order by default. Ask for more per request, and fit calibrations with the same reads.
- Content-free priors (asking each question about `N/A`) and a running batch prior were measured and not kept: both lowered accuracy on skewed tasks.
- Reads multiply the prompts per question (two for noul, one per order for choice), but they share the state, which is prefilled once per request.
- Answers move slightly with batch composition and prefix caching, as described in [Reproducibility](#reproducibility). On K2-Horizon-3.7B, two runs of the same reads differed by 0.001 to 0.014 in mean probability and 0.02 to 0.08 at the 99th percentile, and the top answer changed on 0.3% to 4% of noul and choice questions and on 5% to 8% of the score questions, whose level probabilities are flat.
...
- The server tells from the chat template and its reasoning parser whether answers could start inside a reasoning block, using the parser's reasoning tags in the generation prompt and in the template's own rendering of a finished answer. Reasoning that shows neither is not detected, so check `prompt_token_ids` for such a model. Templates whose generation prompt leaves a block open are served only when their own reply without reasoning closes an empty block before the answer, and refused otherwise.
