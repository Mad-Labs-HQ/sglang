...
<table style={{width: "100%", borderCollapse: "collapse", tableLayout: "fixed"}}>
  ...
  <tbody>
      ...
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.05)"}}>Path of a JSON reads config for <code>/v1/systemone</code>: <code>default_reads</code>, how questions are read when a request sends no <code>x_read_setup</code>, and <code>max_choice_orders</code>, the most orders a choice question may be read in. Without it, questions get the single read of <code>/v1/decisions</code> and requests may ask for up to 8 orders. Calibrations are sent by clients per question. See [Decision models](/docs/supported-models/decision_models#calibration).</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.02)"}}>`None`</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.05)"}}>Type: str</td>
    </tr>
        <tr>
