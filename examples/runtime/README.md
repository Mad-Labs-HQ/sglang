...
* `systemone/reads.json`: A reads config for `/v1/systemone`, the System One compatible decision route: by default it asks yes or no questions in both orders and counts every case of the labels, and it lets requests ask for up to 8 option rotations. Launch with `--decision-reads-config`; see [Decision models](https://docs.sglang.io/docs/supported-models/decision_models#calibration).
## Engine
